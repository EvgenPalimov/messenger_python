...
class MessageProcessor(threading.Thread, metaclass=ServerMaker):
    ...
    def process_clients_message(self, message: dict, client: socket.socket):
        ...
        # Если это запрос публичного ключа пользователя.
            ...
            # Может быть, что ключа ещё нет (пользователь никогда не логинился, то тогда шлём ошибку - 400)
            if response[DATA]:
                try:
                    send_message(client, response)
                except OSError:
    ...
    def user_authorization(self, message: dict, sock: socket.socket):
        ...
        LOGGER.debug(f'Запущен процесс авторизации пользователя - {message[USER]}.')
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            response = RESPONSE_400
            response[ERROR] = 'Имя пользователя уже занято.'
            try:
                LOGGER.debug(f'Имя пользователя занято, отправитель - {response}.')
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            try:
                LOGGER.debug(f'Неисзвестнный пользователь, отправитель - {response}.')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
